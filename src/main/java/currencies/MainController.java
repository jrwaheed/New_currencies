...
*/
    ...
    private void updateCurrencyFromMap (Map<String,BigDecimal> currencyMap) throws SQLException {
        for (int i = 1; i < currencyMap.size(); i++) {
            ...
            String sql = "INSERT INTO " + newCurrency.getCombo() + "(base, ticker, value) " +
                    "VALUES( " + newCurrency.getBase() + ", " + newCurrency.getTicker() +", " + newCurrency.getValue() +")";
                    ...
                    CallableStatement stmt = conn.prepareCall(sql);)
            {
                stmt.execute();
                stmt.close();
                System.out.println("Successfully updated currencies");
            } catch
            (SQLException ex) {
